...
public class UniformCrossover<T, C extends Chromosome<T>> implements CrossoverStrategy<C> {
	...
	public UniformCrossover(ChromosomeFactory<T, C> factory, double mixingRatio) {
		if (mixingRatio < 0.0 || mixingRatio > 1.0) throw new IllegalArgumentException("mixingRatio must be in [0,1]");
